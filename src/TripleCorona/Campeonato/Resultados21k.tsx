...
import { resultados21k } from './resultados21kData'
...
const obtenerEdadMinima = (categoria: string) => {
  const rango = formatearCategoria(categoria)
  const numeroInicial = Number.parseInt(rango.split('-')[0], 10)
  return Number.isNaN(numeroInicial) ? Number.MAX_SAFE_INTEGER : numeroInicial
}

function Resultados21k() {
  ...
  const categorias = useMemo(() => {
    const categoriasUnicas = [...new Set(resultados21k.map((fila) => fila.categoria))]
  ...
  const categoriasDisponibles = useMemo(() => {
    ...
    const prefijoCategoria = filtroGenero === 'F' ? '2F' : '2M'
  ...
  const resultadosFiltrados = useMemo(() => {
    const resultadosBase = resultados21k.filter((fila) => {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl rounded-2xl border border-lime-200 bg-white p-4 shadow-md sm:p-6">
      ...
      <div className="mb-5 grid grid-cols-1 gap-3 rounded-xl border border-lime-100 bg-lime-50/40 p-3 sm:grid-cols-3 sm:p-4">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-semibold uppercase tracking-widest text-neutral-700">Género</span>
          <select
            value={filtroGenero}
      ...
      <p className="mt-3 text-center text-sm text-neutral-600">
        Mostrando {resultadosFiltrados.length === 0 ? 0 : indiceInicio + 1}-{Math.min(indiceFin, resultadosFiltrados.length)} de {resultadosFiltrados.length} resultados filtrados ({resultados21k.length} totales)
      </p>
    </section>
  )
}

export default Resultados21k