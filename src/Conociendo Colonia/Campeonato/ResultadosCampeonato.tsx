...
const extraerResultados = (texto: string): Resultado[] => {
  ...
  return coincidencias.map((coincidencia, indice) => {
    const finalFila = (coincidencia.index ?? 0) + coincidencia[0].length
    const inicioSiguienteFila = coincidencias[indice + 1]?.index ?? textoResultados.length
    const puntajes = textoResultados.slice(finalFila, inicioSiguienteFila).match(/\d+/g) ?? []
...
const normalizarTexto = (texto: string) =>
  texto.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')

function ResultadosCampeonato({ modalidad, archivo }: ResultadosCampeonatoProps) {
  ...
  useEffect(() => {
    ...
    const cargarResultados = async () => {
      try {
        ...
        if (activo) {
          setResultados(paginas.flatMap(extraerResultados))
          setEstado('listo')
        }
      } catch {
  ...
  const categorias = [...new Set(resultados.map((resultado) => resultado.categoria))].sort((primera, segunda) => primera.localeCompare(segunda, undefined, { numeric: true }))
    ...
    <section className="mx-auto w-full max-w-6xl px-4 py-6">
      <div className="border border-lime-200 bg-white p-4 shadow-md sm:p-6">
        <header className="mb-5 text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-lime-700">Conociendo Colonia MTB</p>
          <h3 className="mt-1 text-2xl font-extrabold text-neutral-900 sm:text-3xl">Campeonato {modalidad}</h3>
          <a href={archivo} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm font-semibold text-lime-700 underline hover:text-lime-900">Ver clasificación oficial en PDF</a>
        </header>
        <div className="mb-5 grid gap-3 border border-lime-100 bg-lime-50/50 p-3 sm:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm font-semibold text-neutral-800">Buscar corredor<input value={busqueda} onChange={(event) => setBusqueda(event.target.value)} placeholder="Nombre o apellido" className="border border-lime-300 bg-white px-3 py-2 font-normal outline-none focus:border-lime-600" /></label>
          <label className="flex flex-col gap-1 text-sm font-semibold text-neutral-800">Categoría<select value={categoria} onChange={(event) => setCategoria(event.target.value)} className="border border-lime-300 bg-white px-3 py-2 font-normal outline-none focus:border-lime-600"><option>Todas</option>{categorias.map((opcion) => <option key={opcion}>{opcion}</option>)}</select></label>
        </div>
